...
# Event Storming Visual Diagram to Scenario Tables Skill

Use esta skill quando o usuário fornecer um diagrama visual de Event Storming e pedir para transformá-lo em tabelas descritivas, fáceis de ler, revisar, editar, exportar para planilhas ou converter em backlog.

**Relacionadas:**

- `event-storming` — conduzir/documentar a tempestade e timeline (PlantUML) **antes**, se ainda não houver diagrama
- `ddd-linguagem-e-contextos` / `ddd-integracao-contextos` — Bounded Contexts e integração; BC **pode** ser preenchido depois dessas skills (não é obrigatório nesta etapa)
- `levantamento-requisitos` / `task-refinement` — requisitos e backlog após as tabelas

A skill deve focar em:
...
- erros
- compensações
- continuidade entre funcionalidades

A skill **não deve exigir** bounded context nesta etapa. Se BCs já existirem
(via `ddd-linguagem-e-contextos`), podem ser anotados como enriquecimento.

---

# 1. Objetivo

Transformar diagramas visuais de Event Storming em documentação tabular clara e manipulável.

A saída deve conter:

1. Uma tabela consolidada de políticas reutilizáveis.
2. Uma lista de funcionalidades identificadas.
3. Uma tabela principal por funcionalidade.
4. Cenários alternativos abaixo da tabela principal.
5. Indicação clara de qual etapa originou cada cenário alternativo.
6. Indicação clara de quando a leitura deve continuar em outra funcionalidade.
7. Lista de ambiguidades, hipóteses e pontos que precisam de validação humana.

---

# 2. Entrada esperada

A entrada pode estar em:

- arquivo `.svg`
- arquivo `.drawio.svg`
- imagem exportada do diagrama
- texto extraído do Event Storming
- descrição livre do fluxo
- múltiplos diagramas
- combinação de diagrama e texto complementar

Quando a entrada for visual, o objetivo não é apenas transcrever textos, mas reconstruir uma visão funcional e sequencial do fluxo.
